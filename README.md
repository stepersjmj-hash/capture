...
- **macOS**: `Mcapture-<버전>-mac.dmg` 를 열어 `Mcapture.app` 을 응용 프로그램에 끌어 넣는다.
  ...
  설정 › `로그인 시 자동 실행` 을 켜면 로그인할 때 메뉴 막대에 뜬다.
...
| 도구 | 키 | 설명 |
|---|---|---|
| 영역 | `M` | **기본 도구.** `Ctrl+A` 로 이미지 전체 선택. 드래그로 사각 선택 영역 (`Shift` 정사각형, 안쪽 드래그 = 영역 이동, `Esc` 선택 지우기). 잡은 영역을 **우클릭**하면 `이 영역으로 자르기`(= `Enter`) · `선택 영역 색 채우기` · `선택 영역 테두리 추가` · `선택 지우기` |
| 선택 | `V` · `Space` | 클릭으로 선택 → 드래그 이동, 방향키 1px(Shift 10px) 이동, `Delete` 삭제. 선택하면 그 항목의 색·굵기·글자 크기가 툴바에 표시되고, 바꾸면(칩·스핀·휠) 그 항목에 적용. 텍스트는 더블클릭으로 수정, **우클릭**하면 색 변경·삭제(텍스트는 테두리·수정도) |
| 사각형 | `S` | 드래그로 테두리 상자 |
...
- **macOS 는 `Ctrl` 을 `⌘` 로 읽는다** (`⌘Z` 되돌리기 · `⌘S` 저장 · `⌘W` 닫기 …). 앱 안의 툴팁·`F1`
  치트시트도 `⌘` 로 표시된다. 도구 글자 키(`V` `M` `S` `U` `T` `F`)는 한글 입력 상태에서도 동작한다.
...
로그인 시 자동 실행(Windows 는 레지스트리 `Run`, macOS 는 `~/Library/LaunchAgents` 의 LaunchAgent —
시스템 설정 › 로그인 항목의 "백그라운드에서 허용" 에 표시됨), 시작 시 업데이트 확인.
