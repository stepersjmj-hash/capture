...
| 도구 | 키 | 설명 |
|---|---|---|
| 영역 | `S` | **기본 도구.** 드래그로 사각 선택 영역 (`Shift` 정사각형, 안쪽 드래그 = 영역 이동, `Esc` 선택 지우기). 잡은 영역을 **우클릭**하면 `이 영역으로 자르기`(= `Enter`) · `선택 영역 색 채우기` · `선택 영역 테두리 추가` · `선택 지우기` |
| 선택 | `V` | 클릭으로 선택 → 드래그 이동, 방향키 1px(Shift 10px) 이동, `Delete` 삭제. 선택하면 그 항목의 색·굵기·글자 크기가 툴바에 표시되고, 바꾸면(칩·스핀·휠) 그 항목에 적용. 텍스트는 더블클릭으로 수정, **우클릭**하면 색 변경·삭제(텍스트는 테두리·수정도) |
| 사각형 | `R` | 드래그로 테두리 상자 |
...
- `Ctrl+Z` / `Ctrl+Y` 되돌리기·다시 실행, `Esc` 는 입력 중인 텍스트 취소 → 선택 영역 지우기 →
  선택 해제 → 창 닫기 순.
